...
const WhiteListTable = ({ mockData }) => {
  ...
  const removeFormDeviceWhitelist = async (id: string) => {
    console.log("calling this api", token, id)
    try {
              ...
              return (
                <tr {...row.getRowProps()}>
                  ...
                  <td {...row.cells[6].getCellProps()}>
                    <DeleteButton
                      onClick={() => removeFormDeviceWhitelist(row.cells[0].value)}
                    >Delete</DeleteButton>
                  </td>
